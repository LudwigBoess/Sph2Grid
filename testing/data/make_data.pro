...
;
; You need a (the) glass file from the Wikipage. A chaotic minimum
; energy state is not easy to come by.
;
; Some routines from Klaus' library are needed:
; * readold.pro - to read glass file
; * write_head.pro
; * add_block.pro
;
...
pro make_box, npart, debug=debug
...
; get positions 
;    fin = './glass'
;    readold, fin, tmp, pos, ttmp
;    tmp = 0
;    ttmp = 0
;    pos *= boxsize / max(pos)   ; scale to boxsize
;    npart = n_elements(pos) / 3D
...
; make velocities (the hard part) 
    ...
    v2 = 2.0*Etherm/mass*X_turb         ; total v^2 of particles
